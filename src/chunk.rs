...
/// without retaining their text. Callers take back the writer to finish each block's framing.
...
/// Reconstruct each block's text alongside its chunk indices so scan results map back to rows.
/// Blocks retain first-seen order; their chunk indices are ordered by `split_idx`.
...
mod tests {
    ...
    fn block_writer_matches_reconstruction() {
        ...
        for pieces in cases {
            ...
            assert_eq!(String::from_utf8(writer.into_inner()).unwrap(), reconstruct(&pieces));
        }
    }
    ...
    fn block_writer_handles_many_pieces_with_bounded_tail() {
        ...
        assert_eq!(String::from_utf8(writer.into_inner()).unwrap(), reconstruct(&refs));
    }
