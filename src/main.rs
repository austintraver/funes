...
enum Cmd {
    ...
    Recall {
        ...
        /// Restrict to a harness: claude | codex | pi | hermes | copilot.
        #[arg(long)]
    ...
    Index {
        ...
        path: Option<String>,
        /// Override harness auto-detection for PATH: claude | codex | pi | hermes | copilot.
        #[arg(long)]
        harness: Option<String>,
            ...
            let roots: Vec<(PathBuf, Option<Harness>)> = match path {
                ...
                None => {
                    if !std::io::stdin().is_terminal() {
                        return Err(anyhow!(
                            "automated `funes index` needs a target — pass a path or `--harness <claude|codex|pi|hermes|copilot>`; \
