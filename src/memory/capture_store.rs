...
impl Captured {
    pub(crate) fn new() -> anyhow::Result<Self> {
        let dir = Arc::new(tempfile::tempdir_in(super::dataset::staging_root())?);
    ...
    fn insert(&self, location: OPath, file: CapturedFile) {
        self.state.files.lock().unwrap().insert(location, file);
    }

    fn remove(&self, location: &OPath) {
        self.state.files.lock().unwrap().remove(location);
    }
    ...
    fn next_path(&self) -> OPath {
        ...
        OPath::from(format!("f{}", self.state.next.fetch_add(1, Ordering::Relaxed)))
    }

    async fn record(&self, location: OPath, physical: OPath) -> OSResult<()> {
        let mut meta = self.state.local.head(&physical).await?;
        meta.location = location.clone();
        let file = CapturedFile {
            path: self.state.local.path_to_filesystem(&physical)?,
            physical,
            meta,
            _dir: self.state.dir.clone(),
        };
        self.insert(location, file);
        Ok(())
    }
}
...
impl OSObjectStore for CaptureStore {
    async fn put_opts(&self, location: &OPath, payload: PutPayload, _opts: PutOptions) -> OSResult<PutResult> {
        ...
        let result = self.captured.state.local.put(&physical, payload).await?;
    ...
    async fn put_multipart_opts(
        ...
        let physical = self.captured.next_path();
        let upload = self.captured.state.local.put_multipart(&physical).await?;
    ...
    async fn get_opts(&self, location: &OPath, options: GetOptions) -> OSResult<GetResult> {
        let hit = self.captured.get(location);
        match hit {
            Some(file) => {
                let result = self.captured.state.local.get_opts(&file.physical, options).await?;
...
mod tests {
    ...
    async fn put_is_captured_not_forwarded() {
        ...
        store
            ...
            .await
            .unwrap();
        assert!(store.captured.get(&p).is_some(), "write must be captured");
        assert!(
    ...
    async fn copy_lands_in_capture_never_underlying() {
        ...
        assert!(store.captured.get(&to).is_some());
        assert!(
    ...
    async fn disk_write_failure_exposes_no_capture() {
        ...
        let root = store.captured.state.dir.path();
