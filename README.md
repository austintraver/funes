...
Code, Codex, pi, Hermes, Copilot CLI, and VS Code and lets any agent recall the past decisions, rationale, and findings.
...
- **One memory across your agents.** Index Claude Code, Codex, pi, Hermes, Copilot CLI, and VS Code into a single memory;
...
Your memory isn't tied to one tool. Because Claude Code, Codex, pi, Hermes, Copilot CLI, and VS Code all index into a single
...
VS Code version-3 chat files are available through `funes index --harness vscode` or an explicit path.
