...
Reads workspace and profile chats from the standard `Code` and `Code - Insiders` user-data
roots under `~/Library/Application Support` on macOS, `$APPDATA` on Windows, or
`$XDG_CONFIG_HOME` (default `~/.config`) on Linux.

For custom or portable installations and copied sessions, use `funes index <path> --harness vscode`.
The path can name a native version-3 `.json`/`.jsonl` session file, a `chatSessions` directory,
workspace storage, or a user-data/profile root. Pending responses are picked up on a later run
after they finish.
...
| Flag | Meaning |
| --- | --- |
| `--harness <name>` | Override auto-detection for a path, or (with no path) target one harness's roots: `claude \| codex \| copilot \| pi \| hermes \| vscode`. |
...
```
~/.claude/projects, ~/.codex/sessions, ~/.copilot/session-state, ~/.pi/agent/sessions, ~/.hermes/state.db, VS Code chat stores   (or a .parquet trace)
