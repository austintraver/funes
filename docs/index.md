...
sessions. Pending responses are picked up on a later run after they finish.
...
| Flag | Meaning |
| --- | --- |
| `--harness <name>` | Override auto-detection for a path, or (with no path) target one harness's dir: `claude \| codex \| copilot \| pi \| hermes \| vscode`. |
