...
Copilot honors `COPILOT_HOME` when set (default `~/.copilot`). VS Code searches the `Code` and
`Code - Insiders` directories under `~/Library/Application Support` on macOS, `$APPDATA` on
Windows, or `$XDG_CONFIG_HOME` (default `~/.config`) on Linux, including workspace and profile chats.
...
VS Code accepts native version-3 `.json`/`.jsonl` chat files and chat-store directories.
Use `funes index <path> --harness vscode` for custom or portable installations and copied sessions.
Unfinished responses are indexed after they finish.
