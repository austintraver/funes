...
// Clients on protocol revisions before 2026-07-28 are the ones the SDK would give a session, and
// a session dies with the process. This pins the server's sessionless mode against the case that
// motivated it: a service manager restarting funes under clients that stay connected.
#[tokio::test]
async fn http_restart_is_invisible_to_an_initialized_client() -> Result<()> {
    let home = tempfile::tempdir()?;
...
struct HttpServer {
    home: PathBuf,
    /// Everything after `mcp` except the listen address, so a restart serves the same way.
    args: Vec<OsString>,
    process: Child,
    address: SocketAddr,
    url: String,
...
impl HttpServer {
    async fn new(home: &Path, memory: Option<&Path>, extra: &[&str]) -> Result<Self> {
        ...
        let args: Vec<OsString> = memory
            .map(OsString::from)
            .into_iter()
            .chain(extra.iter().map(OsString::from))
            .collect();
        let (process, url) = Self::spawn(&home, &args, SocketAddr::from(([127, 0, 0, 1], 0))).await?;
        let address = url
            .strip_prefix("http://")
            .and_then(|rest| rest.strip_suffix("/mcp"))
            .context("listening URL is http://<address>/mcp")?
            .parse()?;
